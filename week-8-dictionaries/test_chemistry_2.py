...
from chemistry import tg_make_periodic_table, tg_compute_molar_mass
...
def test_tg_make_periodic_table():
    """Verify that the tg_make_periodic_table function works correctly.
    Parameters: none
    Return: nothing
    """
    # Call the tg_make_periodic_table function and store the returned
    # dictionary in a variable named periodic_table_dict.
    periodic_table_dict = tg_make_periodic_table()

    # Verify that the tg_make_periodic_table function returns a dictionary.
    assert isinstance(periodic_table_dict, dict), \
        "tg_make_periodic_table function must return a dictionary: " \
...
def test_parse_formula():
    ...
    Return: nothing
    """
    # Call the tg_make_periodic_table function
    ...
    # Call the tg_compute_molar_mass function four times and
...
def test_tg_compute_molar_mass():
    """Verify that the tg_compute_molar_mass function works correctly.
    ...
    # Call the tg_compute_molar_mass function
    # and verify that it returns a number.
    molar_mass = tg_compute_molar_mass([["O",2]], periodic_table_dict)
    assert isinstance(molar_mass, int) or isinstance(molar_mass, float), \
        "tg_compute_molar_mass function must return a number: " \
        f" expected a number but found a {type(molar_mass)}"

    # Call the tg_compute_molar_mass function four times and
    # verify that it returns the correct number each time.
    assert tg_compute_molar_mass([], periodic_table_dict) == 0
    assert tg_compute_molar_mass([["O",2]], periodic_table_dict) \
            == approx(31.9988)
    assert tg_compute_molar_mass([["C",6],["H",6]], periodic_table_dict) \
            == approx(78.11184)
    assert tg_compute_molar_mass([["C",13],["H",16],["N",2],["O",2]],

...
from receipt import tg_read_dictionary
from os import path
...
def test_tg_read_dictionary():
    """Verify that the tg_read_dictionary function works correctly.
    Parameters: none
    Return: nothing
    """
    PRODUCT_NUM_INDEX = 0

    # Verify that the tg_read_dictionary function uses its filename
    # parameter by doing the following:
    # 1. Get a filename for a file that doesn't exist.
    # 2. Call the tg_read_dictionary function with the filename.
    # 3. Verify that the open function inside the tg_read_dictionary
    #    function raises a FileNotFoundError.
    filename = mktemp(dir=".", prefix="not", suffix=".csv")
    with pytest.raises(FileNotFoundError):
        tg_read_dictionary(filename, PRODUCT_NUM_INDEX)
        pytest.fail("tg_read_dictionary function must use its filename parameter")

    # Call the tg_read_dictionary function and store the returned
    # dictionary in a variable named products_dict.
    filename = path.join(path.dirname(__file__), "products.csv")
    products_dict = tg_read_dictionary(filename, PRODUCT_NUM_INDEX)

    # Verify that the tg_read_dictionary function returns a dictionary.
    assert isinstance(products_dict, dict), \
        "tg_read_dictionary function must return a dictionary:" \
